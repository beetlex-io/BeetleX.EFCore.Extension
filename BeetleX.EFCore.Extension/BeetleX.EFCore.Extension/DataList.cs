...
namespace BeetleX.EFCore.Extension
{
    ...
    public struct DBExecute
    {
        ...
        public static implicit operator DBExecute(ValueTuple<DbContext, string, string, string, string, string> context)
        {
            ...
            ((SQL)context.Item2).Execute(context.Item1);
            ((SQL)context.Item3).Execute(context.Item1);
            ((SQL)context.Item4).Execute(context.Item1);
            ((SQL)context.Item5).Execute(context.Item1);
            ((SQL)context.Item6).Execute(context.Item1);
            return result;
        }
        ...
        {
            DBExecute result = new DBExecute();
            context.Item2.Execute(context.Item1);
            context.Item3.Execute(context.Item1);
            context.Item4.Execute(context.Item1);
            context.Item5.Execute(context.Item1);
            context.Item6.Execute(context.Item1);
            return result;
        }
    ...
    public struct DBExecute<T> where T : struct
    {
